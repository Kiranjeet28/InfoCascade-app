...
function findCurrentAndNextClass(classes: ClassSlot[]): {
  ...
  for (let i = 0; i < todayClasses.length; i++) {
    ...
    if (currentMin >= start && currentMin < start + 50) {
      current = cls;
...
function WeekMiniCard({
  ...
  return (
    ...
    >
      ...
      <Text
        numberOfLines={3}
        style={{ fontSize: 10, color: colors.textSecondary, lineHeight: 13 }}
      >
        {subject}
      </Text>
    </View>
      ...
      >
        ...
        >
          <AppIcon
            ...
            name={!hasProfile ? "account" : "alert-circle"}
            size={48}
            color={colors.textSecondary}
          />
        </View>
      ...
      >
        <Animated.View style={{ opacity: fadeAnim }}>
          ...
          <View>
            ...
            {/* Compact Week Overview (no horizontal scroll) */}
            <View
              style={{
                backgroundColor: colors.surface,
                borderRadius: 18,
            ...
            >
              <View style={{ flexDirection: "row", gap: 10 }}>
                {WEEK_DAYS.map((day) => {
                  ...
                  const dayClasses = TIME_SLOTS.map((t) => ({
                    time: t,
                    cls: getClassForSlot(day, t),
                  })).filter(({ cls }) => cls && !cls.data.freeClass);

                  const firstThree = dayClasses.slice(0, 3);
                  const overflow = Math.max(
                    0,
                    dayClasses.length - firstThree.length,
                  );
                    ...
                    >
                      ...
                      <View style={{ gap: 6 }}>
                        {firstThree.length === 0 ? (
                          ...
                          firstThree.map(({ time, cls }) => {
                            const typeColor = getTypeColor(cls!);
                            const subject =
                              cls!.data.subject ??
                              cls!.data.entries?.[0]?.subject ??
                              "Class";
              ...
              >
                Tap a day to view its full schedule above.
              </Text>
            </View>
          </View>
        </Animated.View>
