...
import { NotificationDebugger } from "../../utils/notifications-debug";
...
export default function HomeScreen() {
  ...
  const handleTestNotification = async () => {
    ...
    try {
      await NotificationDebugger.testScheduleNotification();
      showNotification({
        title: "✅ Test Notification Scheduled",
        body: "Check notification tray in ~5 seconds",
        type: "info",
      });
    } catch (error) {
      console.error("[HomeScreen] Error testing notification:", error);
      showNotification({
        title: "❌ Test Failed",
        body: "Check logs and ensure permissions are granted",
        type: "info",
      });
    } finally {
              ...
              {Platform.OS !== "web" && (
                <View style={{ alignItems: "center", gap: 4 }}>
                  ...
                  >
                    <AppIcon
                      family="Ionicons"
                      name={testingNotification ? "hourglass" : "flask"}
                      size={20}
                      color={colors.primary}
                    />
                  </TouchableOpacity>
