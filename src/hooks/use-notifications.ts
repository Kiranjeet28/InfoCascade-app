...
async function scheduleDayNotifications(
  classes: ClassSlot[],
  day: string,
  dayOffset: number = 0,
): Promise<string[]> {
  ...
  const scheduleNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        console.log(
          `[useNotifications] Cancelling ${pastNotifications.length} past notifications`,
        );
      ...
      const ids = await scheduleDayNotifications(
        groupData.classes,
        currentDay,
        0,
      );
